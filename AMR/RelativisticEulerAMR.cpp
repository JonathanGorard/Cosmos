#include "RelativisticEulerAMR.hpp"

void RelativisticEulerAMR::computeFORCETimeStepAMR(vector<RelativisticEulerStateVector> & currentCells, double cellSpacing, double timeStep,
...
#pragma omp parallel for
    for (int i = 0; i < cellCount; i++)
    {
        if (AMRStructure[i])
        {
            ...
            vector<double> leftFluxVector = RelativisticEulerFirstOrderSolver::computeXFORCEFlux(currentCellsWithBoundary[i], currentCellsWithBoundary[i + 1],
                                                                                                 cellSpacing, timeStep, materialParameters);
            vector<double> rightFluxVector = RelativisticEulerFirstOrderSolver::computeXFORCEFlux(currentCellsWithBoundary[i + 1], currentCellsWithBoundary[i + 2],
                                                                                                  cellSpacing, timeStep, materialParameters);
...
void RelativisticEulerAMR::computeXFORCETimeStep2DAMR(vector<vector<RelativisticEulerStateVector> > & currentCells, double cellSpacing, double timeStep,
    ...
    for (int i = 0; i < rowCount; i++)
    {
        for (int j = 0; j < columnCount; j++)
        {
            if (AMRStructure[i][j])
            {
                ...
                vector<double> leftFluxVector = RelativisticEulerFirstOrderSolver::computeXFORCEFlux(currentCellsWithBoundary[i + 1][j],
                                                                                                     currentCellsWithBoundary[i + 1][j + 1], cellSpacing, timeStep,
                                                                                                     materialParameters);
                vector<double> rightFluxVector = RelativisticEulerFirstOrderSolver::computeXFORCEFlux(currentCellsWithBoundary[i + 1][j + 1],
                                                                                                      currentCellsWithBoundary[i + 1][j + 2], cellSpacing, timeStep,
                                                                                                      materialParameters);
...
void RelativisticEulerAMR::computeYFORCETimeStep2DAMR(vector<vector<RelativisticEulerStateVector> > & currentCells, double cellSpacing, double timeStep,
                                                      vector<vector<bool> > AMRStructure, EulerMaterialParameters materialParameters)
{
    long rowCount = currentCells.size();
    long columnCount = currentCells[0].size();
    ...
    for (int i = 0; i < rowCount; i++)
    {
        for (int j = 0; j < columnCount; j++)
        {
            if (AMRStructure[i][j])
            {
                ...
                vector<double> topFluxVector = RelativisticEulerFirstOrderSolver::computeYFORCEFlux(currentCellsWithBoundary[i][j + 1],
                                                                                                    currentCellsWithBoundary[i + 1][j + 1], cellSpacing, timeStep,
                                                                                                    materialParameters);
                vector<double> bottomFluxVector = RelativisticEulerFirstOrderSolver::computeYFORCEFlux(currentCellsWithBoundary[i + 1][j + 1],
                                                                                                       currentCellsWithBoundary[i + 2][j + 1], cellSpacing, timeStep,
                                                                                                       materialParameters);
...
void RelativisticEulerAMR::computeSLICTimeStepAMR(vector<RelativisticEulerStateVector> & currentCells, double cellSpacing, double timeStep, double bias,
                                                  int slopeLimiter, vector<bool> AMRStructure, EulerMaterialParameters materialParameters)
{
    long cellCount = currentCells.size();
...
#pragma omp parallel for
    for (int i = 0; i < cellCount; i++)
    {
        if (AMRStructure[i])
        {
            ...
            vector<double> leftFluxVector = RelativisticEulerSecondOrderSolver::computeXSLICFlux(currentCellsWithBoundary[i], currentCellsWithBoundary[i + 1],
                                                                                                 currentCellsWithBoundary[i + 2], currentCellsWithBoundary[i + 3],
                                                                                                 cellSpacing, timeStep, bias, slopeLimiter, materialParameters);
            vector<double> rightFluxVector = RelativisticEulerSecondOrderSolver::computeXSLICFlux(currentCellsWithBoundary[i + 1], currentCellsWithBoundary[i + 2],
...
void RelativisticEulerAMR::computeXSLICTimeStep2DAMR(vector<vector<RelativisticEulerStateVector> > & currentCells, double cellSpacing, double timeStep, double bias,
    ...
    for (int i = 0; i < rowCount; i++)
    {
        for (int j = 0; j < columnCount; j++)
        {
            if (AMRStructure[i][j])
            {
                ...
                vector<double> leftFluxVector = RelativisticEulerSecondOrderSolver::computeXSLICFlux(currentCellsWithBoundary[i + 2][j],
                ...
                vector<double> rightFluxVector = RelativisticEulerSecondOrderSolver::computeXSLICFlux(currentCellsWithBoundary[i + 2][j + 1],
                                                                                                      currentCellsWithBoundary[i + 2][j + 2],
...
void RelativisticEulerAMR::computeYSLICTimeStep2DAMR(vector<vector<RelativisticEulerStateVector> > & currentCells, double cellSpacing, double timeStep, double bias,
                                                     int slopeLimiter, vector<vector<bool> > AMRStructure, EulerMaterialParameters materialParameters)
{
    long rowCount = currentCells.size();
    long columnCount = currentCells[0].size();
    ...
    for (int i = 0; i < rowCount; i++)
    {
        for (int j = 0; j < columnCount; j++)
        {
            if (AMRStructure[i][j])
            {
                ...
                vector<double> topFluxVector = RelativisticEulerSecondOrderSolver::computeYSLICFlux(currentCellsWithBoundary[i][j + 2],
                                                                                                    currentCellsWithBoundary[i + 1][j + 2],
                                                                                                    currentCellsWithBoundary[i + 2][j + 2],
                                                                                                    currentCellsWithBoundary[i + 3][j + 2], cellSpacing, timeStep,
                                                                                                    bias, slopeLimiter, materialParameters);
                vector<double> bottomFluxVector = RelativisticEulerSecondOrderSolver::computeYSLICFlux(currentCellsWithBoundary[i + 1][j + 2],
                                                                                                       currentCellsWithBoundary[i + 2][j + 2],
...
tuple<vector<RelativisticEulerStateVector>, vector<RelativisticEulerStateVector>, vector<bool>, vector<bool>>
RelativisticEulerAMR::solveLevel1AMR(vector<RelativisticEulerStateVector> initialCells, double cellSpacing, double CFLCoefficient, double finalTime,
                                     double AMRTolerance, int order, EulerMaterialParameters materialParameters)
{
    long cellCount = initialCells.size();
    
    vector<RelativisticEulerStateVector> coarseCurrentCells(cellCount);
    vector<RelativisticEulerStateVector> fineCurrentCells(cellCount * 2);
...
#pragma omp parallel for
    ...
    {
        double coarseTimeStep = RelativisticEulerSolvers::computeStableTimeStep(coarseCurrentCells, cellSpacing, CFLCoefficient, coarseCurrentTime, finalTime,
                                                                                fineTotalIteration, materialParameters);
        
        vector<RelativisticEulerStateVector> coarseCurrentCellsWithBoundary = RelativisticEulerSolvers::insertBoundaryCells(coarseCurrentCells, 1);
...
#pragma omp parallel for
        ...
        if (refined)
        {
            ...
            {
                double fineTimeStep = RelativisticEulerSolvers::computeStableTimeStep(fineCurrentCells, cellSpacing * 0.5, CFLCoefficient, fineCurrentTime,
...
#pragma omp parallel for
        ...
        vector<RelativisticEulerStateVector> fineCurrentCellsWithBoundary = RelativisticEulerSolvers::insertBoundaryCells(fineCurrentCells, 1);
...
#pragma omp parallel for
    ...
    return tuple<vector<RelativisticEulerStateVector>, vector<RelativisticEulerStateVector>, vector<bool>, vector<bool>>(coarseCurrentCells, fineCurrentCells,
                                                                                                                         coarseAMRStructure, fineAMRStructure);
}

tuple<vector<vector<RelativisticEulerStateVector> >, vector<vector<RelativisticEulerStateVector> >, vector<vector<bool> >, vector<vector<bool> >>
RelativisticEulerAMR::solve2DLevel1AMR(vector<vector<RelativisticEulerStateVector> > initialCells, double cellSpacing, double CFLCoefficient, double finalTime,
                                       double AMRTolerance, int order, EulerMaterialParameters materialParameters)
{
    long rowCount = initialCells.size();
    long columnCount = initialCells[0].size();
    
    vector<vector<RelativisticEulerStateVector> > coarseCurrentCells(rowCount, vector<RelativisticEulerStateVector>(columnCount));
    vector<vector<RelativisticEulerStateVector> > fineCurrentCells(rowCount * 2, vector<RelativisticEulerStateVector>(columnCount * 2));
...
#pragma omp parallel for
    ...
    while (coarseCurrentTime < finalTime)
    {
        double coarseTimeStep = RelativisticEulerSolvers::computeStableTimeStep2D(coarseCurrentCells, cellSpacing, CFLCoefficient, coarseCurrentTime, finalTime,
                                                                                  fineTotalIteration, materialParameters);
        
        vector<vector<RelativisticEulerStateVector> > coarseCurrentCellsWithBoundary = RelativisticEulerSolvers::insertBoundaryCells2D(coarseCurrentCells, 1);
...
#pragma omp parallel for
        for (int i = 0; i < rowCount; i++)
        {
            for (int j = 0; j < columnCount; j++)
            {
                ...
                vector<double> bottomConservedVariableVector = coarseCurrentCellsWithBoundary[i + 2][j + 2].computeConservedVariableVector(materialParameters);
...
#pragma omp parallel for
        ...
        if (refined)
        {
            ...
            while (fineCurrentTime < (coarseCurrentTime + coarseTimeStep - pow(10.0, -4.0)))
            {
                double fineTimeStep = RelativisticEulerSolvers::computeStableTimeStep2D(fineCurrentCells, cellSpacing * 0.5, CFLCoefficient, fineCurrentTime,
                                                                                        coarseCurrentTime + coarseTimeStep, fineTotalIteration, materialParameters);
...
#pragma omp parallel for
        ...
        vector<vector<RelativisticEulerStateVector> > fineCurrentCellsWithBoundary = RelativisticEulerSolvers::insertBoundaryCells2D(fineCurrentCells, 1);
...
#pragma omp parallel for
    ...
    return tuple<vector<vector<RelativisticEulerStateVector> >, vector<vector<RelativisticEulerStateVector> >, vector<vector<bool> >,
    vector<vector<bool> >>(coarseCurrentCells, fineCurrentCells, coarseAMRStructure, fineAMRStructure);
}

tuple<vector<RelativisticEulerStateVector>, vector<RelativisticEulerStateVector>, vector<RelativisticEulerStateVector>, vector<bool>, vector<bool>, vector<bool>>
RelativisticEulerAMR::solveLevel2AMR(vector<RelativisticEulerStateVector> initialCells, double cellSpacing, double CFLCoefficient, double finalTime,
                                     double AMRTolerance, int order, EulerMaterialParameters materialParameters)
{
    long cellCount = initialCells.size();
    
    vector<RelativisticEulerStateVector> coarseCurrentCells(cellCount);
    vector<RelativisticEulerStateVector> intermediateCurrentCells(cellCount * 2);
    vector<RelativisticEulerStateVector> fineCurrentCells(cellCount * 4);
...
#pragma omp parallel for
        ...
        if (refined)
        {
            ...
            {
                double intermediateTimeStep = RelativisticEulerSolvers::computeStableTimeStep(intermediateCurrentCells, cellSpacing * 0.5, CFLCoefficient,
                                                                                              intermediateCurrentTime, coarseCurrentTime + coarseTimeStep,
                                                                                              fineTotalIteration, materialParameters);
                
                vector<RelativisticEulerStateVector> intermediateCurrentCellsWithBoundary = RelativisticEulerSolvers::insertBoundaryCells(intermediateCurrentCells, 1);
...
#pragma omp parallel for
                ...
                if (refinedLevel2)
                {
                    ...
                    {
                        double fineTimeStep = RelativisticEulerSolvers::computeStableTimeStep(fineCurrentCells, cellSpacing * 0.25, CFLCoefficient, fineCurrentTime,
...
#pragma omp parallel for
    ...
    return tuple<vector<RelativisticEulerStateVector>, vector<RelativisticEulerStateVector>, vector<RelativisticEulerStateVector>, vector<bool>, vector<bool>,
...
tuple<vector<vector<RelativisticEulerStateVector> >, vector<vector<RelativisticEulerStateVector> >, vector<vector<RelativisticEulerStateVector> >,
vector<vector<bool> >, vector<vector<bool> >, vector<vector<bool> >> RelativisticEulerAMR::solve2DLevel2AMR(vector<vector<RelativisticEulerStateVector> > initialCells, double cellSpacing, double CFLCoefficient, double finalTime,
                                       double AMRTolerance, int order, EulerMaterialParameters materialParameters)
{
    long rowCount = initialCells.size();
    long columnCount = initialCells[0].size();
    ...
    vector<vector<RelativisticEulerStateVector> > coarseCurrentCells(rowCount, vector<RelativisticEulerStateVector>(columnCount));
    vector<vector<RelativisticEulerStateVector> > intermediateCurrentCells(rowCount * 2, vector<RelativisticEulerStateVector>(columnCount * 2));
    vector<vector<RelativisticEulerStateVector> > fineCurrentCells(rowCount * 4, vector<RelativisticEulerStateVector>(columnCount * 4));
...
#pragma omp parallel for
        ...
        if (refined)
        {
            ...
            while (intermediateCurrentTime < (coarseCurrentTime + coarseTimeStep - pow(10.0, -4.0)))
            {
                double intermediateTimeStep = RelativisticEulerSolvers::computeStableTimeStep2D(intermediateCurrentCells, cellSpacing * 0.5, CFLCoefficient,
                                                                                                intermediateCurrentTime, coarseCurrentTime + coarseTimeStep,
                                                                                                fineTotalIteration, materialParameters);
                
                vector<vector<RelativisticEulerStateVector> > intermediateCurrentCellsWithBoundary =
                RelativisticEulerSolvers::insertBoundaryCells2D(intermediateCurrentCells, 1);
...
#pragma omp parallel for
                ...
                if (refinedLevel2)
                {
                    ...
                    while (fineCurrentTime < (intermediateCurrentTime + intermediateTimeStep - pow(10.0, -4.0)))
                    {
                        double fineTimeStep = RelativisticEulerSolvers::computeStableTimeStep2D(fineCurrentCells, cellSpacing * 0.25, CFLCoefficient,
                                                                                                fineCurrentTime, intermediateCurrentTime + intermediateTimeStep,
                                                                                                fineTotalIteration, materialParameters);
...
#pragma omp parallel for
        ...
        vector<vector<RelativisticEulerStateVector> > intermediateCurrentCellsWithBoundary = RelativisticEulerSolvers::insertBoundaryCells2D(intermediateCurrentCells, 1);
...
#pragma omp parallel for
    ...
    return tuple<vector<vector<RelativisticEulerStateVector> >, vector<vector<RelativisticEulerStateVector> >, vector<vector<RelativisticEulerStateVector> >,
